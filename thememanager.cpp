...
QIcon ThemeManager::createDynamicIcon(const QString& svgPath, const QSize& size) {
    QIcon icon;

    // Normal state
    icon.addPixmap(coloredPixmap(svgPath, m_iconColor, size), QIcon::Normal, QIcon::Off);

    // Hover state (Active)
    icon.addPixmap(coloredPixmap(svgPath, m_hoverColor, size), QIcon::Active, QIcon::Off);

    // Pressed state (Selected)
    icon.addPixmap(coloredPixmap(svgPath, m_pressedColor, size), QIcon::Selected, QIcon::Off);

    // Disabled state
    icon.addPixmap(coloredPixmap(svgPath, m_disabledColor, size), QIcon::Disabled, QIcon::Off);

    return icon;
}

void ThemeManager::setupDynamicAction(QAction* action, const QString& svgPath, const QSize& size) {
    if (!action) return;
...
// Color getters
QColor ThemeManager::getCurrentIconColor() const {
    return m_iconColor;
}

QColor ThemeManager::getCurrentHoverColor() const {
    return m_hoverColor;
}

QColor ThemeManager::getCurrentPressedColor() const {
    return m_pressedColor;
}

QColor ThemeManager::getCurrentDisabledColor() const {
