...
QJsonObject EtagController::fetchEtags(const QMap<QString, QString>& urls, bool c) {
  ...
  for (auto it = urls.cbegin(); it != urls.cend(); ++it) {
    ...
    req.setRawHeader("User-Agent", "AutoClicker/2.0");
...
void EtagController::updateUrlMaps() {
  ...
  // Patchnotes her zaman aynı
  urlmap[urlHeader + "patchnotes.md"] = "patchnotes";
  rurlmap["patchnotes"] = urlHeader + "patchnotes.md";

  // Theme dosyalarını version'a göre ayarla
  if (ISBETA) {
    urlmap[urlHeader + "themes_beta/dark.qss"] = "t_dark";
    urlmap[urlHeader + "themes_beta/light.qss"] = "t_light";
    rurlmap["t_dark"] = urlHeader + "themes_beta/dark.qss";
    rurlmap["t_light"] = urlHeader + "themes_beta/light.qss";
  } else {
...
QString EtagController::getKeyFromRelativePath(const QString& relativePath) {
  ...
  if (relativePath.endsWith("themes/dark.qss") || relativePath.endsWith("themes_beta/dark.qss")) return "t_dark";
  if (relativePath.endsWith("themes/light.qss") || relativePath.endsWith("themes_beta/light.qss")) return "t_light";
  return "";
}
