...
void IconUtils::setupToolbarIcons(QWidget* toolbar, const QMap<QString, QString>& actionIconMap) {
    if (!toolbar) return;
    ...
    Logger::instance().sInfo(QString("Toolbar icons setup: %1/%2 successful").arg(successCount).arg(totalCount));
}

void IconUtils::setupButtonIcons(QWidget* parent, const QMap<QString, QString>& buttonIconMap) {
    if (!parent) return;
    ...
    for (auto it = buttonIconMap.constBegin(); it != buttonIconMap.constEnd(); ++it) {
    ...
    Logger::instance().sInfo(QString("Button icons setup: %1/%2 successful").arg(successCount).arg(totalCount));
}

bool IconUtils::setupIcon(QAction* action, const QString& iconPath, const QSize& size) {
    ...
    if (validateIconPath(iconPath)) {
        ...
        Logger::instance().sWarning("Fallback icon created for action: " + action->objectName());
        return false;
    }
}

bool IconUtils::setupIcon(QPushButton* button, const QString& iconPath, const QSize& size) {
    ...
    if (validateIconPath(iconPath)) {
        ...
        Logger::instance().sWarning("Fallback icon created for button: " + button->objectName());
        return false;
    }
}

bool IconUtils::setupIcon(QToolButton* button, const QString& iconPath, const QSize& size) {
    if (!button) return false;
    ...
    if (validateIconPath(iconPath)) {
        ...
        button->setIcon(fallbackIcon);
        button->setIconSize(size);
        Logger::instance().sWarning("Fallback icon created for tool button: " + button->objectName());
        return false;
    }
}
...
QStringList IconUtils::findAvailableIcons(const QString& iconDirectory) const {
    ...
    if (!dir.exists()) {
        Logger::instance().fsWarning("Icon directory does not exist: " + iconDirectory);
        return iconFiles;
    }
...
QIcon IconUtils::createFallbackIcon(const QString& text, const QSize& size, const QColor& bgColor) {
    QPixmap pixmap(size);
...
void ExampleWidget::setupAllIcons() {
    ...
    ThemeManager::instance().setupDynamicAction(ui->actionPlay, iconsPath + "/play.svg", QSize(16, 16));
    ThemeManager::instance().setupDynamicAction(ui->actionStop, iconsPath + "/stop.svg", QSize(16, 16));
    ThemeManager::instance().setupDynamicAction(ui->actionSettings, iconsPath + "/settings.svg", QSize(16, 16));

    ThemeManager::instance().setupDynamicButton(ui->buttonStart, iconsPath + "/play.svg", QSize(24, 24));
    ThemeManager::instance().setupDynamicButton(ui->buttonStop, iconsPath + "/stop.svg", QSize(24, 24));
    ...
    IconUtils::instance().setupIcon(ui->actionSpecial, iconsPath + "/special.svg");
