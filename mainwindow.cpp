...
MainWindow::MainWindow(const QJsonObject& settings,
  ...
  activeMacro = MacroManager::instance()
                    .getMacroById(getSetting("ActiveMacro").toInt(1))
                    .value();
...
void MainWindow::addActionToTable(MacroAction a) {
  ...
  ui->actionsTable->setItem(row, 0,
                            new QTableWidgetItem(QString::number(a.order)));
  ...
  QString msbtnstr = a.mouse_button.has_value()
  ...
  ui->actionsTable->setItem(row, 4,
                            new QTableWidgetItem(QString::number(a.repeat)));
  ui->actionsTable->setItem(row, 5,
