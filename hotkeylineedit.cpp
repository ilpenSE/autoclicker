    ...
    , m_inFocusEvent(false)
{
    ...
    connect(m_updateTimer, &QTimer::timeout, this, &HotkeyLineEdit::updateDisplay);
...
QString HotkeyLineEdit::getHotkey() const
{
    return m_finalHotkey;
}
...
{
    if (!m_capturing) { startCapture(); return; }
...
bool HotkeyLineEdit::isValidKey(int key) const
{
    ...
    if ((key >= Qt::Key_A && key <= Qt::Key_Z) ||
        (key >= Qt::Key_0 && key <= Qt::Key_9)) return true;
